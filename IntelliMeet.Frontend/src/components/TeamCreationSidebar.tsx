...
const TeamCreationSidebar: React.FC<TeamCreationSidebarProps> = ({ onAllMeetingsClick }) => {
    return (
        <aside className="hidden xl:block w-[190px] bg-bg-surface-pure border-l border-stroke-primary h-screen fixed right-0 top-0 flex flex-col gap-4 sm:gap-5 p-4 sm:p-5 overflow-y-auto z-[999]">
            <div className="flex flex-col gap-2 rounded-12 sm:rounded-16 shadow-card p-0 bg-bg-surface-pure">
                <div className="py-1 px-2 flex items-center justify-center">
                    <p className="flex-1 font-inter font-medium text-xs leading-4 text-text-secondary m-0">
                        My Meetings
                    </p>
                </div>
                <button
                    onClick={onAllMeetingsClick}
                    className="flex gap-[6px] items-center p-2 rounded-8 cursor-pointer relative bg-[rgba(22,163,74,0.1)] hover:bg-[rgba(22,163,74,0.15)] transition-colors w-full text-left"
                ...
                >
                    ...
                    <span className="font-inter font-medium text-sm text-text-secondary tracking-[-0.084px] leading-5">
                        All Meetings
                    </span>
                    <div className="absolute bottom-[-14px] left-0 right-0 h-0.5 bg-primary-500" aria-hidden="true"></div>
                </button>
            </div>

            <div className="flex flex-col gap-0">
                <div className="flex items-center justify-between mb-2">
                    <p className="font-inter font-medium text-xs leading-4 text-text-secondary m-0 uppercase">
                        TEAMS
                    </p>
                    <button 
                        className="bg-transparent border-none w-[18px] h-[18px] flex items-center justify-center cursor-pointer p-0 hover:opacity-70 transition-opacity"
                ...
                <div className="flex flex-col gap-2 items-center py-4 sm:py-5">
                    ...
                    <p className="font-inter font-normal text-xs leading-4 text-text-secondary text-center max-w-[186px] m-0">
                        Create teams to organize meetings
                    </p>
                    <button 
                        className="bg-bg-surface-lv1 border border-bg-surface-pure rounded-8 px-3 sm:px-[10px] py-2 sm:py-[6px] flex items-center gap-1 cursor-pointer font-inter font-medium text-sm text-text-secondary tracking-[-0.084px] leading-5 hover:bg-bg-surface-lv2 transition-colors"
